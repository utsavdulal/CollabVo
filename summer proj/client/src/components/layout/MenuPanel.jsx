...
export function MenuPanel({ open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute right-0 top-0 h-full w-72 max-w-[85vw] bg-white shadow-2xl flex flex-col justify-between">
        <div>
          <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100">
            <p className="font-bold text-sm text-zinc-900">{user.name || 'Account Menu'}</p>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="py-2 px-2">
            {items.map(({ label, icon: Icon, path }) => (
              <button
                ...
                type="button"
                onClick={() => go(path)}
                className="flex w-full items-center gap-3 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 hover:text-zinc-900 transition-colors"
              >
